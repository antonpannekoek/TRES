...
def powerlaw_distr(m_min, m_max, slope):
    if slope == -1 or slope == 0:
        sys.exit("slope of powerlaw distribution incorrect")
    slope1 = slope + 1
...
def beta_distr_SSOs(lower, upper, mass):  # (Bowler et al. 2020)
    ...
    return beta_distr_SSOs(
        lower, upper, mass
    )  # pick another sample within given bounds
...
class Generate_initial_triple:
    ...
    def __init__(
        self,
        inner_primary_mass_max,
        inner_primary_mass_min,
        inner_secondary_mass_max,
        inner_secondary_mass_min,
        outer_mass_min,
        outer_mass_max,
        inner_mass_ratio_max,
        inner_mass_ratio_min,
        ...
        outer_semi_latus_rectum_min,
        inner_semi_latus_rectum_max,
        outer_semi_latus_rectum_max,
        inner_ecc_max,
        inner_ecc_min,
        outer_ecc_max,
        outer_ecc_min,
        ...
        if inner_primary_mass_distr == 5:
            ...
            while mass_convergence is False:
                mass_convergence = self.generate_mass_and_semi_eggleton(
                    inner_primary_mass_max,
                    inner_primary_mass_min,
                    inner_secondary_mass_min,
                    outer_mass_min,
            ...
            self.inner_ecc = self.generate_ecc_1d(
                inner_ecc_max, inner_ecc_min, inner_ecc_distr, self.inner_secondary_mass
            )
            self.outer_ecc = self.generate_ecc_1d(
                outer_ecc_max, outer_ecc_min, outer_ecc_distr, self.outer_mass
            )
            ...
            while orbit_convergence is False:
    ...
    def generate_mass(
        ...
        if inner_primary_mass_max == inner_primary_mass_min:
            ...
            if inner_primary_mass_distr == 1:  # Scalo 1986
                self.inner_primary_mass = new_scalo_mass_distribution(
                    1, inner_primary_mass_max
                )[0]
                ...
                self.inner_primary_mass = new_miller_scalo_mass_distribution(
                    1, inner_primary_mass_max
                )[0]
                while self.inner_primary_mass < inner_primary_mass_min:
                    self.inner_primary_mass = new_miller_scalo_mass_distribution(
                ...
                self.inner_primary_mass = new_salpeter_mass_distribution(
                    1, inner_primary_mass_min, inner_primary_mass_max
                )[0]
            elif inner_primary_mass_distr == 4:  # Flat in log space
                self.inner_primary_mass = new_flat_mass_distribution(
                    1, inner_primary_mass_min, inner_primary_mass_max
                )[0]
            elif (
                inner_primary_mass_distr == 5
            ):  # Eggleton 2009, 399, 1471, Salpeter-like with turnover at low masses
                self.inner_primary_mass = eggleton_mass_distr(
                    inner_primary_mass_min, inner_primary_mass_max
                )
            elif (
                inner_primary_mass_distr == 6
            ):  # Salpeter with slope 2.3 -> Kroupa for M>0.5
                self.inner_primary_mass = powerlaw_distr(
                    inner_primary_mass_min, inner_primary_mass_max, -2.3
                )
            else:  # Kroupa 2001
                self.inner_primary_mass = new_kroupa_mass_distribution(
                    1, mass_min=inner_primary_mass_min, mass_max=inner_primary_mass_max
                )[0]
        ...
        if inner_mass_ratio_max == inner_mass_ratio_min:
            ...
            if inner_mass_ratio_distr == 1:  # Kroupa 2001
                self.inner_secondary_mass = new_kroupa_mass_distribution(
                    1,
            ...
            #                self.inner_secondary_mass = new_kroupa_mass_distribution(1, mass_min=inner_secondary_mass_min, mass_max=inner_primary_mass)[0]
                ...
                self.inner_secondary_mass = powerlaw_distr(
                    ...
                    m_max=inner_secondary_mass_max,
                    slope=-1.31,
                )
            else:  # flat distribution
                inner_mass_ratio = flat_distr(
                    max(
                        inner_mass_ratio_min,
                        inner_secondary_mass_min / self.inner_primary_mass,
                    ),
                    min(
                        inner_mass_ratio_max,
                        inner_secondary_mass_max / self.inner_primary_mass,
                    ),
                )
                self.inner_secondary_mass = inner_mass_ratio * self.inner_primary_mass
        ...
        if outer_mass_ratio_max == outer_mass_ratio_min:
            ...
            self.outer_mass = outer_mass_ratio * (
                self.inner_primary_mass + self.inner_secondary_mass
            )
        else:
            if outer_mass_ratio_distr == 1:  # Kroupa 2001
                self.outer_mass = new_kroupa_mass_distribution(
                    1, mass_min=outer_mass_min, mass_max=outer_mass_max
                )[0]
            elif outer_mass_ratio_distr == 2:  # Galicher et al 2016
                self.outer_mass = powerlaw_distr(
                    m_min=outer_mass_min, m_max=outer_mass_max, slope=-1.31
                )
            else:  # flat distribution
                inner_mass_tot = self.inner_primary_mass + self.inner_secondary_mass
                outer_mass_ratio = flat_distr(
                    max(outer_mass_ratio_min, outer_mass_min / inner_mass_tot),
                    min(outer_mass_ratio_max, outer_mass_max / inner_mass_tot),
                )
                self.outer_mass = outer_mass_ratio * inner_mass_tot

    def generate_semi_and_ecc(
        self,
        inner_semi_max_orig,
        inner_semi_min_orig,
        outer_semi_max_orig,
        ...
        self.inner_ecc = self.generate_ecc_1d(
            inner_ecc_max, inner_ecc_min, inner_ecc_distr, inner_secondary_mass
        )
        self.outer_ecc = self.generate_ecc_1d(
            outer_ecc_max, outer_ecc_min, outer_ecc_distr, outer_mass
        )
        ...
        if inner_semi_max == inner_semi_min:
            ...
            if inner_semi_distr == 1:  # Constant
                ...
                self.inner_semi = 0.0 | units.RSun
                while (
                    self.inner_semi < inner_semi_min or self.inner_semi > inner_semi_max
                ):
                    self.inner_ecc = self.generate_ecc_1d(
                        inner_ecc_max,
                        inner_ecc_min,
                ...
                inner_semi_distr == 4
            ):  # Rizzuto et al 2013, 436, 1694, Lognormal mu=10^0.95AU, sigma=1.35
                self.inner_semi = 0.0 | units.RSun
                while (
                    self.inner_semi < inner_semi_min or self.inner_semi > inner_semi_max
                ):
                    ...
                    if (
                        self.inner_semi < 0.5 | units.RSun
                        or self.inner_semi > 5e8 | units.RSun
                    ):  # truncation of Gaussian wings
                        self.inner_semi = 0.0 | units.RSun
                ...
                while (
                    ...
                    logP = (random_nr * c_s + logP_min**0.45) ** (1.0 / 0.45)
                    P0 = 10**logP | units.day
                    M_inner = self.inner_primary_mass + self.inner_secondary_mass
                    self.inner_semi = (
                        (P0 / 2.0 / np.pi) ** 2 * M_inner * constants.G
                    ) ** (1.0 / 3.0)

            elif inner_semi_distr == 6:  # flat distr (uniform)
                self.inner_semi = (
                    flat_distr(
                        inner_semi_min.value_in(units.RSun),
                        inner_semi_max.value_in(units.RSun),
                    )
                    | units.RSun
                )
            elif inner_semi_distr == 7:  # Galicher 2016: powerlaw, slope -0.61
                self.inner_semi = powerlaw_distr(
                    inner_semi_min, inner_semi_max, slope=-0.61
                )
                ...
                self.inner_semi = (
                    log_flat_distr(
                        inner_semi_min.value_in(units.RSun),
                        maximal_semi.value_in(units.RSun),
                    )
                    | units.RSun
                )
        ...
        if outer_semi_max == outer_semi_min:
            ...
            if outer_semi_distr == 1:  # Constant
                if REPORT_USER_WARNINGS_TPS:
                    ...
                    print(
                        "--a_min option to set the value of the semi-major axis in the outer binary"
                    )
                self.outer_semi = outer_semi_min
            elif outer_semi_distr == 2:  # Tokovinin Lognormal mu=10^5.5d, sigma=2.3
                self.outer_semi = 0.0 | units.RSun
                while (
                    self.outer_semi < outer_semi_min or self.outer_semi > outer_semi_max
                ):
                    self.outer_ecc = self.generate_ecc_1d(
                        outer_ecc_max, outer_ecc_min, outer_ecc_distr, outer_mass
                    )
                ...
                while (
                    ...
                    if (
                        self.outer_semi < 0.5 | units.RSun
                        or self.outer_semi > 5e8 | units.RSun
                    ):  # truncation of Gaussian wings
                        self.outer_semi = 0.0 | units.RSun
                ...
                while (
                    ...
                    self.outer_semi = (
                        (P0 / 2.0 / np.pi) ** 2 * mass_tot * constants.G
                    ) ** (1.0 / 3.0)

            elif outer_semi_distr == 6:  # flat distr (uniform)
                self.outer_semi = (
                    flat_distr(
                        outer_semi_min.value_in(units.RSun),
                        outer_semi_max.value_in(units.RSun),
                    )
                    | units.RSun
                )
            elif outer_semi_distr == 7:  # Galicher 2016: powerlaw, slope -0.61
                self.outer_semi = powerlaw_distr(
                    outer_semi_min, outer_semi_max, slope=-0.61
                )

            else:  # log flat distribution
                if (
                    outer_semi_min > outer_semi_max
                ):  # possible for extreme eccentricities
                    return False
                self.outer_semi = (
                    log_flat_distr(
                        outer_semi_min.value_in(units.RSun),
        ...
        if (
            ...
            and inner_semi_min_orig == outer_semi_min_orig
            and inner_semi_max == outer_semi_max
            and self.outer_semi < self.inner_semi
            and inner_ecc_distr == outer_ecc_distr
            and inner_ecc_min == outer_ecc_min
            and inner_ecc_max == outer_ecc_max
        ):
    ...
    def generate_ecc_1d(self, ecc_max, ecc_min, ecc_distr, mass):
        ...
        if ecc_max == ecc_min:
            ...
            if ecc_distr == 1:  # Constant
                if REPORT_USER_WARNINGS_TPS:
                    print(
                        "TPS::generate_ecc: unambiguous choice of constant eccentricity"
                    )
                    print(
                        "--E_min and --e_min option to set the value of the eccentricity"
                    )
                return ecc_min
    ...
    def generate_incl(self, incl_max, incl_min, incl_distr):
        ...
        if incl_max == incl_min:
            ...
            if incl_distr == 1:  # Constant
                if REPORT_USER_WARNINGS_TPS:
                    print(
                        "TPS::generate_incl: unambiguous choice of constant relative inclination"
                    )
                    print(
                        "--i_min option to set the value of the relative inclination in the inner triple"
                    )
                self.incl = incl_min
    ...
    def generate_aop(
        ...
        if inner_aop_max == inner_aop_min:
            ...
            if inner_aop_distr == 1:  # Constant
                if REPORT_USER_WARNINGS_TPS:
                    print(
                        "TPS::generate_aop: unambiguous choice of constant argument of pericenter"
                    )
                    print(
                        "--G_min option to set the value of the argument of pericenter of the inner binary"
                    )
                self.inner_aop = inner_aop_min
        ...
        if outer_aop_max == outer_aop_min:
            ...
            if outer_aop_distr == 1:  # Constant
                if REPORT_USER_WARNINGS_TPS:
                    ...
                    print(
                        "--g_min option to set the value of the argument of pericenter of the outer binary"
                    )
                self.outer_aop = outer_aop_min
            else:  # Uniform distribution
    ...
    def generate_loan(self, inner_loan_max, inner_loan_min, inner_loan_distr):
        ...
        if inner_loan_max == inner_loan_min:
            ...
            if inner_loan_distr == 0:  # Circular uniform distribution
                ...
                if REPORT_USER_WARNINGS_TPS:
                    print(
                        "TPS::generate_loan: unambiguous choice of constant longitude of ascending nodes"
                    )
                    print(
                        "--O_min option to set the value of the argument of pericenter of the inner binary"
                    )
                self.inner_loan = inner_loan_min
    ...
    def generate_mass_and_semi_eggleton(
        self,
        inner_primary_mass_max,
        inner_primary_mass_min,
        inner_secondary_mass_min,
        ...
        U0_mass = [0.0, 0.01, 0.09, 0.32, 1.0, 3.2, 11, 32, np.inf]  # solar mass
        ...
        if U < U0:
            ...
            P0 = 1.0e5 * V**2 / (1 - V) ** 2.5 | units.day
            ...
            self.inner_semi = ((P_bin / 2.0 / np.pi) ** 2 * M_bin * constants.G) ** (
                1.0 / 3.0
            )
            self.outer_semi = ((P0 / 2.0 / np.pi) ** 2 * Mt * constants.G) ** (
                1.0 / 3.0
            )

            if (
                self.inner_primary_mass < inner_primary_mass_min
                or self.inner_primary_mass > inner_primary_mass_max
            ):
                return False
    ...
    def print_triple(self):
        print("\nTriple - ")
        print(
            "m =", self.inner_primary_mass, self.inner_secondary_mass, self.outer_mass
        )
    ...
    def print_triple_short(self):
        print(
            self.inner_primary_mass.value_in(units.MSun),
            self.inner_secondary_mass.value_in(units.MSun),
            self.outer_mass.value_in(units.MSun),
            end=" ",
        )
        print(
            self.inner_semi.value_in(units.RSun),
            self.outer_semi.value_in(units.RSun),
            end=" ",
        )
...
def evolve_model(
    ...
    while i_n < number:
        triple_system = Generate_initial_triple(
            inner_primary_mass_max,
            inner_primary_mass_min,
            inner_secondary_mass_max,
            inner_secondary_mass_min,
        ...
        if EXCLUDE_SSO:
            if (min_mass > triple_system.inner_secondary_mass) or (
                min_mass > triple_system.outer_mass
            ):
                if REPORT_TPS:
                    print(
                        "non-star secondary & tertiary included: ",
                        triple_system.inner_secondary_mass,
                        triple_system.outer_mass,
                    )
                continue
        ...
        if tr.correct_params == False:
            ...
            if tr.has_tertiary_donor():
                ...
                if include_circ:
                    ...
                    max_nr_tries_ecc = 10
                    while (
                        i_ecc < max_nr_tries_ecc and tr.mass_transfer_at_initialisation
                    ):
                        i_ecc += 1
                        new_ecc = triple_system.generate_ecc_1d(
                            triple_system.inner_ecc,
                            inner_ecc_min,
                            inner_ecc_distr,
                            triple_system.inner_secondary_mass,
                        )
...
def print_distr(
    ...
    print(
        stop_at_eccentric_stable_mass_transfer,
        "\t Stop at eccentric stable mass transfer",
    )
    print(stop_at_unstable_mass_transfer, "\t Stop at unstable mass transfer")
    print(
        stop_at_eccentric_unstable_mass_transfer,
        "\t Stop at eccentric unstable mass transfer",
    )
...
def test_initial_parameters(
    ...
    if (inner_primary_mass_min < min_mass) or (
    ...
    if inner_primary_mass_max < inner_primary_mass_min:
        sys.exit(
            "error: maximum inner primary mass smaller than minimum in primary mass"
        )

    if inner_secondary_mass_max < inner_secondary_mass_min:
        sys.exit(
            "error: maximum inner secondary mass smaller than minimum in secondary mass"
        )

    if outer_mass_max < outer_mass_min:
        sys.exit("error: maximum outer mass smaller than minimum in outer mass")

    if (inner_mass_ratio_min < 0.0) or (inner_mass_ratio_max > 1.0):
    ...
    if (outer_mass_ratio_min < 0.0) or (outer_mass_ratio_max > 1.0):
    ...
    if outer_semi_max < outer_semi_min:
        sys.exit(
            "error: maximum outer separation smaller than minimum outer separation"
        )

    if inner_semi_min > outer_semi_max:
        sys.exit(
            "error: maximum outer separation smaller than minimum inner separation - no overlap for inner and outer orbit"
        )

    if (inner_ecc_min < 0.0) or (inner_ecc_max > 1.0):
        sys.exit("error: inner eccentricity not in allowed range [0,1]")

    if (outer_ecc_min < 0.0) or (outer_ecc_max > 1.0):
    ...
    if (incl_min < 0.0) or (incl_max > np.pi):
        sys.exit("error: relative inclination not in allowed range [0, pi]")

    if incl_max < incl_min:
        sys.exit(
            "error: maximum relative inclination smaller than minimum relative inclination"
        )
    ...
    if inner_aop_max < inner_aop_min:
        sys.exit(
            "error: maximum inner argument of pericenter smaller than minimum argument of pericenter"
        )

    if outer_aop_max < outer_aop_min:
        sys.exit(
            "error: maximum outer argument of pericenter smaller than minimum argument of pericenter"
        )

    if (inner_loan_min < -1 * np.pi) or (inner_loan_max > np.pi):
        sys.exit(
            "error: inner longitude of ascending node not in allowed range [-pi,pi]"
        )

    if inner_loan_max < inner_loan_min:
        sys.exit(
            "error: maximum inner longitude of ascending node smaller than minimum argument of pericenter"
        )
