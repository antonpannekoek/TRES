...
def check_for_radiative_damping(stellar_type, mass):
    if (
        ...
        and mass.value_in(units.MSun) >= MINIMUM_MASS_FOR_RADIATIVE_DAMPING_MSUN
    ):
        return True
...
def tidal_friction_constant(
    ...
    if USE_RADIATIVE_DAMPING:  ### radiative damping ###
        E2 = 1.592e-09 * pow(
            mass.value_in(units.MSun), 2.84
        )  ### Hurley prescription; Zahn, 1977, A&A, 57, 383 and 1975, A&A, 41, 329. ###

        k_div_T_tides = (
            E2
            * pow(1.0 + companion_mass / mass, 5.0 / 6.0)
            * radius
            * numpy.sqrt(constants.G * mass / (semimajor_axis**5))
        )
    ...
    # 		kTradiative_damping = kTradiative_damping/YEAR_LENGTH_IN_SECONDS;	/* This converts (k/T) to units of s^-1 */
        ...
        P_orb = (
            2.0
            * numpy.pi
            * numpy.sqrt((semimajor_axis**3) / (constants.G * (mass + companion_mass)))
        )
        if spin_angular_frequency.value_in(1.0 / units.s) == 0.0:
            ...
            P_tid_s = 1.0 / (
                1e-10
                + numpy.fabs(
                    1.0 / (P_orb.value_in(units.s)) - 1.0 / (P_spin.value_in(units.s))
                )
            )
        ...
        tau_convective = pow(
            (
                convective_envelope_mass
                * convective_envelope_radius
                * (radius - (1.0 / 2.0) * convective_envelope_radius)
            )
            / (3.0 * luminosity),
            1.0 / 3.0,
        )
        ...
        k_div_T_tides = (
            (2.0 / 21.0)
            * (f_convective / tau_convective)
            * (convective_envelope_mass / mass)
        )
        ...
        stellar_type == PLANET or stellar_type == BD
    ):  # based on Fabrycky & Tremaine 2007, appendix
        T_viscous = 0.001 | units.yr
        return amc / T_viscous
    else:  ### degenerate damping -- 1984MNRAS.207..433C ###
        tau_degenerate = 1.3e7 | units.yr
        k_div_T_tides = (
            (1.0 / (3.0 * tau_degenerate))
            * gyration_radius**2
            * pow(
                luminosity.value_in(units.LSun) / mass.value_in(units.MSun), 5.0 / 7.0
            )
        )
