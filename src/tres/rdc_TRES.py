...
def rdc(
    ...
    print_init,
    line_number,
    inner_primary_star_type,
    inner_secondary_star_type,
    outer_star_type,
    inner_primary_star_type_string,
    inner_secondary_star_type_string,
    outer_star_type_string,
    inner_bin_type,
    outer_bin_type,
    inner_bin_type_string,
    outer_bin_type_string,
    triple_type,
    triple_type_string,
):
        ...
        for i, triple in enumerate(triple.history):
            if i == line_number:
                ...
                if triple[0].time > minimum_time_step:
                    print(
                        "Warning: these parameters do not represent a system at birth (ZAMS)."
                    )
    ...
    if (
        ...
        and star_type[inner_primary_star_type_string] != -1
        and inner_primary_star_type != star_type[inner_primary_star_type_string]
    ):
        print("error: two different inner primary star types requested")
        return
    if (
        inner_secondary_star_type != -1
        and star_type[inner_secondary_star_type_string] != -1
        and inner_secondary_star_type != star_type[inner_secondary_star_type_string]
    ):
        print("error: two different inner secondary star types requested")
        return
    if (
        outer_star_type != -1
        and star_type[outer_star_type_string] != -1
        and outer_star_type != star_type[outer_star_type_string]
    ):
        print("error: two different outer star types requested")
        return
    if (
        inner_bin_type > -1
        and bin_type[inner_bin_type_string] > -1
        and inner_bin_type != bin_type[inner_bin_type_string]
    ):
        print("error: two different inner binary types requested")
        return
    if (
        outer_bin_type > -1
        and bin_type[outer_bin_type_string] > -1
        and outer_bin_type != bin_type[outer_bin_type_string]
    ):
        print("error: two different outer binary types requested")
        return
    if (
        triple_type > -1
        and tr_type[triple_type_string] > -1
        and triple_type != tr_type[triple_type_string]
    ):
    ...
    if print_style == 2 or print_style == 3:
        snapshot_string = (
            snapshot_string
            + "number, time, incl, dyn_inst, kozai_type, error_flag, CPU_time,"
        )
        snapshot_string = (
            snapshot_string + " bin_type2, a2, e2, p2, l2, bin_type1, a1, e1, p1, l1,"
        )
        snapshot_string = (
            snapshot_string
            + " m1_donor, m1_stellar_type, m1_mass, m1_spin, m1_radius, m1_core,"
        )
        snapshot_string = (
            snapshot_string
            + " m2_donor, m2_stellar_type, m2_mass, m2_spin, m2_radius, m2_core,"
        )
        snapshot_string = (
            snapshot_string
            + " m3_donor, m3_stellar_type, m3_mass, m3_spin, m3_radius, m3_core,"
        )
        snapshot_string = snapshot_string + "\n"
    triple_number = 0
    ...
    for i, triple in enumerate(triple.history):
        ...
        if print_style == 0:
            ...
            snapshot_string = snapshot_string + print_to_string(
                " bs: ",
                triple[0].child2.bin_type,
                triple[0].child2.semimajor_axis,
                triple[0].child2.eccentricity,
                triple[0].child2.argument_of_pericenter,
            ...
            snapshot_string = snapshot_string + print_to_string(
                " st: ",
                triple[0].child2.child1.is_donor,
                triple[0].child2.child1.stellar_type,
                triple[0].child2.child1.mass,
                triple[0].child2.child1.spin_angular_frequency,
                triple[0].child2.child1.radius,
                triple[0].child2.child1.core_mass,
            )
            snapshot_string = snapshot_string + print_to_string(
                "|",
                triple[0].child2.child2.is_donor,
                triple[0].child2.child2.stellar_type,
                triple[0].child2.child2.mass,
                triple[0].child2.child2.spin_angular_frequency,
                triple[0].child2.child2.radius,
                triple[0].child2.child2.core_mass,
            )
            snapshot_string = snapshot_string + print_to_string(
                "|",
                triple[0].child1.is_donor,
                triple[0].child1.stellar_type,
                triple[0].child1.mass,
                triple[0].child1.spin_angular_frequency,
                triple[0].child1.radius,
                triple[0].child1.core_mass,
            )
            ...
            if print_style == 3:
                snapshot_string = snapshot_string + print_to_string_csv(
                    triple[0].number,
                    triple[0].time.value_in(units.Myr),
                    triple[0].relative_inclination,
                    int(triple[0].dynamical_instability),
                    int(triple[0].kozai_type),
                    int(triple[0].error_flag_secular),
                    triple[0].CPU_time,
                    end=",",
                )
                snapshot_string = snapshot_string + print_to_string_csv(
                    bin_type[triple[0].child2.bin_type],
                    triple[0].child2.semimajor_axis.value_in(units.RSun),
                    triple[0].child2.eccentricity,
                    triple[0].child2.argument_of_pericenter,
                    triple[0].child2.longitude_of_ascending_node,
                ...
                snapshot_string = snapshot_string + print_to_string_csv(
                    int(triple[0].child2.child1.is_donor),
                    triple[0].child2.child1.stellar_type.value_in(units.stellar_type),
                    triple[0].child2.child1.mass.value_in(units.MSun),
                    triple[0].child2.child1.spin_angular_frequency.value_in(
                        1.0 / units.Myr
                    ),
                ...
                snapshot_string = snapshot_string + print_to_string_csv(
                    int(triple[0].child2.child2.is_donor),
                    triple[0].child2.child2.stellar_type.value_in(units.stellar_type),
                    triple[0].child2.child2.mass.value_in(units.MSun),
                    triple[0].child2.child2.spin_angular_frequency.value_in(
                        1.0 / units.Myr
                    ),
                ...
                snapshot_string = snapshot_string + print_to_string_csv(
                    int(triple[0].child1.is_donor),
                    triple[0].child1.stellar_type.value_in(units.stellar_type),
                    triple[0].child1.mass.value_in(units.MSun),
                    triple[0].child1.spin_angular_frequency.value_in(1.0 / units.Myr),
        ...
        if (
            (
                inner_primary_star_type == star_type["all"]
                or triple[0].child2.child1.stellar_type.value_in(units.stellar_type)
                in np.array(inner_primary_star_type)
            )
            and (
                inner_secondary_star_type == star_type["all"]
                or triple[0].child2.child2.stellar_type.value_in(units.stellar_type)
                in np.array(inner_secondary_star_type)
            )
            and (
                outer_star_type == star_type["all"]
                or triple[0].child1.stellar_type.value_in(units.stellar_type)
                in np.array(outer_star_type)
            )
            and (
                inner_bin_type == bin_type["all"]
                or inner_bin_type == bin_type[triple[0].child2.bin_type]
            )
            and (
                outer_bin_type == bin_type["all"]
                or outer_bin_type == bin_type[triple[0].bin_type]
            )
            and (triple_type == tr_type["all"] or triple_type == triple_type_snapshot)
        ):
            correct_system = True
...
def parse_arguments():
    ...
    parser = OptionParser()
    parser.add_option(
        "-f", dest="file_name_root", default="TRES", help="file name [%default]"
    )
    parser.add_option(
        "-S", dest="print_style", type="int", default=2, help="print style [%default]"
    )
    parser.add_option(
        "-F",
        dest="print_full",
    ...
    parser.add_option(
        ...
        type="int",
        default=0,
        help="line number for printing initial conditions [%default]",
    )  # will only do something when print_init = True

    # returns first instance where desired star_type, bin_type & triple_type is reached
    parser.add_option(
        "--st1",
        dest="inner_primary_star_type",
        type="int",
        default=-1,
    ...
    parser.add_option(
        "--st2",
        dest="inner_secondary_star_type",
        type="int",
        default=-1,
    ...
    parser.add_option(
        "--st1str",
        dest="inner_primary_star_type_string",
        default="all",
        help="desired stellar type of inner binary primary star (int) [%default]",
    )
    parser.add_option(
        "--st2str",
        dest="inner_secondary_star_type_string",
        default="all",
        help="desired stellar type of inner binary secondary star (int) [%default]",
    )
    parser.add_option(
        "--st3str",
        dest="outer_star_type_string",
        default="all",
