...
def make_bins(
    ...
    outer_longitude_of_ascending_node,
):
...
def test_initial_parameters(
    ...
    if min(inner_secondary_mass, outer_mass) <= absolute_min_mass:
        ...
        print(
            "substellar objects can be included through EXCLUDE_SSO in TRES_options.py"
        )
        return False, 0, 0
    ...
    if inner_semimajor_axis >= outer_semimajor_axis:
        ...
        return False, 0, 0
    if inner_semimajor_axis < 0.0 | units.RSun:
        print("error: inner separation not in allowed range")
        return False, 0, 0
    if outer_semimajor_axis < 0.0 | units.RSun:
        print("error: outer separation not in allowed range")
        return False, 0, 0

    if (inner_eccentricity < 0.0) or (inner_eccentricity > 1.0):
        print("error: inner eccentricity not in allowed range")
        return False, 0, 0
    if (outer_eccentricity < 0.0) or (outer_eccentricity > 1.0):
    ...
    if (relative_inclination < 0.0) or (relative_inclination > np.pi):
        print("error: relative inclination not in allowed range")
        return False, 0, 0

    if (inner_argument_of_pericenter < -1.0 * np.pi) or (
        inner_argument_of_pericenter > np.pi
    ):
        print("error: inner argument of pericenter not in allowed range")
        return False, 0, 0
    if (outer_argument_of_pericenter < -1.0 * np.pi) or (
        outer_argument_of_pericenter > np.pi
    ):
        print("error: outer argument of pericenter not in allowed range")
        return False, 0, 0

    if (inner_longitude_of_ascending_node < -1.0 * np.pi) or (
        inner_longitude_of_ascending_node > np.pi
    ):
...
def make_particle_sets(
    ...
    correct_params, inner_eccentricity, outer_eccentricity = test_initial_parameters(
        inner_primary_mass,
        inner_secondary_mass,
        outer_mass,
        inner_semimajor_axis,
...
def setup_secular_code(
    triple,
    secular_code,
    stop_at_semisecular_regime,
):
    code_module = secular_code.__module__.split('.')[-2]
    print(f"Using code {code_module}")

    if code_module == "seculartriple":
        ...
        secular_code.parameters.include_1PN_inner_outer_terms = (
            False  ### warning: probably broken
        )
        ...
        secular_code.parameters.check_for_semisecular_regime = (
            stop_at_semisecular_regime
        )
        secular_code.parameters.check_for_semisecular_regime_at_initialisation = (
            stop_at_semisecular_regime
        )
        ...
        secular_code.parameters.threshold_value_of_spin_angular_frequency_for_setting_spin_angular_frequency_dot_moment_of_inertia_plus_wind_changes_zero = (
            1.0e-7 | units.Myr**-1
        )

        secular_code.parameters.include_linear_mass_change = (
            True  # needed for Jspin conservation
        )
        secular_code.parameters.include_linear_radius_change = (
            True  # needed for Jspin conservation
        )
    ...
    elif code_module == "secularmultiple":
        def sectrip_to_secmul(p):
            """
            Unpacks the triple into "binary" particles and "star" particles and
            returns a Particles object of these.
            """
            s = Particles()
            if hasattr(p, "child1"):
                s.add_particles(
                    sectrip_to_secmul(p.child1)
                )
                p.is_binary = True
            if hasattr(p, "child2"):
                s.add_particles(
                    sectrip_to_secmul(p.child2)
                )
                p.is_binary = True
            s.add_particle(p)
            return s
        triple_set = sectrip_to_secmul(triple)
        # triple_set = triple.as_set()
        triple_time = triple_set.time
        if not hasattr(triple_set, "inclination"):
            triple_set.inclination = 0
        ...
        secular_code.parameters.include_quadrupole_order_terms = True
        secular_code.parameters.include_octupole_order_binary_pair_terms = True
        secular_code.parameters.include_octupole_order_binary_triplet_terms = False
