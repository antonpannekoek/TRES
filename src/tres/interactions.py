...
small_numerical_error = 1.0e-10
minimum_eccentricity = 1.0e-5
...
stellar_types_planetary_objects = [
    18,
    19,
] | units.stellar_type  # planets & brown dwarfs

stellar_types_SN_remnants = [
    13,
    14,
    15,
] | units.stellar_type  # remnant types created through a supernova
...
stellar_types_dr = [
    ...
    14,
    15,
] | units.stellar_type  # stars which go through a instantaneous radius change at formation; hertzsprung gap stars (small envelope perturbation) + horizontal branch stars + remnants
...
def stellar_evolution_timescale(star):
    ...
    if star.stellar_type in [0, 1, 7] | units.stellar_type:
        return (
            0.1 * star.mass * nucleair_efficiency * constants.c**2 / star.luminosity
        ).in_(units.Gyr)
    if star.stellar_type in stellar_types_compact_objects:
...
def nuclear_evolution_timescale(star):
    ...
    # t_nuc ~ delta t * R/ delta R, other prescription gave long timescales in
    # SeBa which destables the mass transfer
    if (
        star.time_derivative_of_radius
        ...
        t_nuc = (
            star.radius / star.time_derivative_of_radius
        )  # does not include the effect of mass loss on R
...
def kelvin_helmholds_timescale(star):
    ...
    if REPORT_FUNCTION_NAMES:
        print(
            "KH timescale:",
            (constants.G * star.mass**2 / star.radius / star.luminosity).in_(units.Myr),
        )
...
def lang_spin_angular_frequency(star):
    v_rot = (
        330
        * star.mass.value_in(units.MSun) ** 3.3
        / (15.0 + star.mass.value_in(units.MSun) ** 3.45)
    )
    w = 45.35 * v_rot / star.radius.value_in(units.RSun)
    return w | 1.0 / units.yr
...
def criticial_angular_frequency_CHE(m, Z):
    ...
    omega_at_z_0d004 = np.sum(
        a_coeff * m.value_in(units.MSun) ** mass_power / m.value_in(units.MSun) ** 0.4
    )
    omega_crit = omega_at_z_0d004 / (0.09 * np.log(Z / 0.004) + 1) | 1.0 / units.s
    return omega_crit
...
def copy_outer_orbit_to_inner_orbit(bs, self):
    ...
    if self.is_triple():
        ...
        (bs.accretion_efficiency_wind_child1_to_child2,) = (
            self.triple.accretion_efficiency_wind_child1_to_child2,
        )
        (bs.accretion_efficiency_wind_child2_to_child1,) = (
            self.triple.accretion_efficiency_wind_child2_to_child1,
        )
        (bs.specific_AM_loss_mass_transfer,) = (
            self.triple.specific_AM_loss_mass_transfer,
        )
...
def perform_inner_collision(self):
    if self.is_triple():
        ...
        donor_in_stellar_code = donor.as_set().get_intersecting_subset_in(
            self.stellar_code.particles
        )[0]
        accretor_in_stellar_code = accretor.as_set().get_intersecting_subset_in(
            self.stellar_code.particles
        )[0]
        ...
        critical_spin_angular_frequency = np.sqrt(
            constants.G * donor.mass / donor.radius**3
        )
        donor.spin_angular_frequency = min(
            donor.spin_angular_frequency, critical_spin_angular_frequency
        )

        self.stellar_code.particles.remove_particle(accretor)
        accretor.mass = (
            0 | units.MSun
        )  # necessary for adjust_system_after_ce_in_inner_binary
        ...
        if self.has_donor():
            print(
                ...
                self.triple.child2.eccentricity,
                self.triple.child2.child1.is_donor,
                self.triple.child2.child2.is_donor,
            )
            print(
                self.triple.child1.mass,
                self.triple.semimajor_axis,
                self.triple.eccentricity,
                self.triple.child1.is_donor,
            )
        ...
        donor.spin_angular_frequency = corotating_spin_angular_frequency_binary(
            bs.semimajor_axis, bs.child1.mass, bs.child2.mass
        )
...
def common_envelope_angular_momentum_balance(bs, donor, accretor, self):
    ...
    J_init = (
        np.sqrt(bs.semimajor_axis)
        * (donor.mass * accretor.mass)
        / np.sqrt(donor.mass + accretor.mass)
        * np.sqrt(1 - bs.eccentricity**2)
    )
    J_f_over_sqrt_a_new = (donor.core_mass * accretor.mass) / np.sqrt(
        donor.core_mass + accretor.mass
    )
    J_lost = (
        gamma * (donor.mass - donor.core_mass) * J_init / (donor.mass + accretor.mass)
    )
    sqrt_a_new = max(0.0 | units.RSun**0.5, (J_init - J_lost) / J_f_over_sqrt_a_new)
    ...
    if (donor.core_radius > Rl_donor_new) or (accretor.radius > Rl_accretor_new):
        ...
        donor_in_stellar_code.change_mass(
            -1 * (donor.mass - donor.core_mass + (small_numerical_error | units.MSun)),
            0.0 | units.yr,
        )
        self.copy_from_stellar()
        ...
        corotating_frequency = corotating_spin_angular_frequency_binary(
            a_new, donor.mass, accretor.mass
        )
        ...
        if self.has_donor():
            print(
                self.triple.child2.child1.mass,
                self.triple.child2.child2.mass,
                self.triple.child2.child1.radius,
                self.triple.child2.child2.radius,
                self.triple.child2.semimajor_axis,
                self.triple.child2.eccentricity,
                self.triple.child2.child1.is_donor,
                self.triple.child2.child2.is_donor,
            ...
            print(
                self.triple.child2.child1.core_mass,
                self.triple.child2.child1.mass - self.triple.child2.child1.core_mass,
                self.triple.child2.child1.stellar_type,
            )
            print(
                self.triple.child1.mass,
                self.triple.semimajor_axis,
...
def common_envelope_energy_balance(bs, donor, accretor, self):
    ...
    orb_energy_new = (
        ...
        * (donor.mass - donor.core_mass)
        / (alpha * lambda_donor * donor_radius)
        + donor.mass * accretor.mass / 2 / pericenter_init
    )
...
def double_common_envelope_energy_balance(bs, donor, accretor, self):
    ...
    orb_energy_new = (
        ...
        * (accretor.mass - accretor.core_mass)
        / (alpha * lambda_accretor * accretor_radius)
        + donor.mass * accretor.mass / 2 / pericenter_init
    )
    a_new = donor.core_mass * accretor.core_mass / 2 / orb_energy_new

    Rl_donor_new = (
        roche_radius_dimensionless(donor.core_mass, accretor.core_mass) * a_new
    )
    Rl_accretor_new = (
        roche_radius_dimensionless(accretor.core_mass, donor.core_mass) * a_new
    )
    if REPORT_BINARY_EVOLUTION:
        print("donor:", donor.radius, donor.core_radius, Rl_donor_new)
        print("accretor:", accretor.radius, accretor.core_radius, Rl_accretor_new)
    ...
    if (donor.core_radius > Rl_donor_new) or (accretor.core_radius > Rl_accretor_new):
        ...
        accretor_in_stellar_code.change_mass(
            -1 * (accretor.mass - accretor.core_mass), 0.0 | units.yr
        )
        self.copy_from_stellar()
...
def common_envelope_phase(bs, donor, accretor, self):
    ...
    if (
        donor.stellar_type not in stellar_types_giants
        and accretor.stellar_type not in stellar_types_giants
    ):
        ...
        if self.has_donor():
            ...
            print(
                ...
                roche_radius(bs, bs.child1, self),
                roche_radius(bs, bs.child2, self),
            )
    ...
    if self.which_common_envelope == 0:
        if (
            donor.stellar_type in stellar_types_giants
            and accretor.stellar_type in stellar_types_giants
        ):
            stopping_condition = double_common_envelope_energy_balance(
                bs, donor, accretor, self
            )
        else:
            stopping_condition = common_envelope_energy_balance(
                bs, donor, accretor, self
            )
        ...
        if (
            ...
            stopping_condition = common_envelope_angular_momentum_balance(
                bs, donor, accretor, self
            )
        ...
        if (
            ...
            donor.stellar_type in stellar_types_compact_objects
            or accretor.stellar_type in stellar_types_compact_objects
        ):
            # giant+remnant
            stopping_condition = common_envelope_energy_balance(
                bs, donor, accretor, self
...
def adjust_system_after_ce_in_inner_binary(bs, self):
    ...
    if self.is_triple():
        ...
        a_new = adiabatic_expansion_due_to_mass_loss(
            self.triple.semimajor_axis,
            M_com_after_ce,
            M_com_before_ce,
            M_accretor_after_ce,
            M_accretor_before_ce,
        )
...
def stable_mass_transfer(bs, donor, accretor, self):
    ...
    donor_in_stellar_code.change_mass(
        dm_desired + (small_numerical_error | units.MSun), dt
    )
    ...
    accretor_in_stellar_code.change_mass(-1.0 * dm, -1.0 * dt)
    ...
    if (
        abs(accretion_efficiency - 1.0) > numerical_error
        and abs(Md - Md_new - -1.0 * (Ma - Ma_new)) > numerical_error | units.MSun
    ):
        self.save_snapshot()
    ...
    corotation_spin = corotating_spin_angular_frequency_binary(
        bs.semimajor_axis, donor.mass, accretor.mass
    )
...
def perform_mass_equalisation_for_contact(bs, donor, accretor, self):
    ...
    if donor.mass != accretor.mass:
        ...
        bs.semimajor_axis = (
            bs.semimajor_axis * (donor.mass * accretor.mass / new_mass**2) ** 2
        )
        ...
        self.stellar_code.evolve_model(
            minimum_time_step
        )  # to get updates radii, not just inflation of stars due to accretion
        self.copy_from_stellar()
        ...
        if self.include_CHE:
            ...
            self.channel_to_stellar.copy_attributes(
                ["rotation_period"]
            )  # only defined when include_CHE
        ...
        if bs.child2.is_star:
            ...
            if bs.child1.is_donor:
                if bs.child2.child1.is_donor or bs.child2.child2.is_donor:
                    stopping_condition = outer_mass_transfer(
                        bs, bs.child1, bs.child2, self
                    )
            else:
                detached(bs, self)
...
def q_crit(self, donor, companion):
    ...
    if donor.stellar_type in [9] | units.stellar_type:
        ...
        return 1.0 / 0.625  # following claeys et al. 2014 based on de mink et al 2007
    elif (
        donor.stellar_type in stellar_types_compact_objects
    ):  # eventhough ns & bh shouldn't be donors...
        return 0.25  # 0.628
    elif donor.stellar_type in [18, 19] | units.stellar_type:  # planet or brown dwarf.
        # metzger et al 2012,425,2778,
        return 1.0 * (donor.radius / self.get_size(companion)) ** 3
...
def mass_transfer_stability(binary, self):
    ...
    if self.is_binary(binary):
        ...
        if REPORT_MASS_TRANSFER_STABILITY:
            ...
            print(Js_1, Js_2, Jb, Jb / 3.0)

        Js = max(Js_1, Js_2)
        if Js >= Jb / 3.0:
            if REPORT_MASS_TRANSFER_STABILITY:
                print(
                    "Mass transfer stability: Darwin Riemann instability",
                    Js_1,
                    Js_2,
                    Jb,
                    Jb / 3.0,
                )
            mt1 = -1.0 * binary.child1.mass / dynamic_timescale(binary.child1)
            mt2 = -1.0 * binary.child2.mass / dynamic_timescale(binary.child2)
            ...
            #     binary.is_mt_stable = True
                ...
                binary.child1.stellar_type <= 1 | units.stellar_type
                ...
                mt1 = (
                    -1.0
                    * binary.child1.mass
                    / nuclear_evolution_timescale(binary.child1)
                )
                mt2 = (
                    -1.0
                    * binary.child2.mass
                    / nuclear_evolution_timescale(binary.child2)
                )
                binary.mass_transfer_rate = min(mt1, mt2)  # minimum because mt<0
                binary.is_mt_stable = False
            else:
            ...
            binary.child1.is_donor
            and binary.child1.mass
            > binary.child2.mass * q_crit(self, binary.child1, binary.child2)
        ):
            if REPORT_MASS_TRANSFER_STABILITY:
                print("Mass transfer stability: Mdonor1>Macc*q_crit ")
            binary.mass_transfer_rate = (
                -1.0 * binary.child1.mass / dynamic_timescale(binary.child1)
            )
            binary.is_mt_stable = False
        elif (
            binary.child2.is_donor
            and binary.child2.mass
            > binary.child1.mass * q_crit(self, binary.child2, binary.child1)
        ):
            if REPORT_MASS_TRANSFER_STABILITY:
                print("Mass transfer stability: Mdonor2>Macc*q_crit ")
            binary.mass_transfer_rate = (
                -1.0 * binary.child2.mass / dynamic_timescale(binary.child2)
            )
            binary.is_mt_stable = False
            ...
            binary.mass_transfer_rate = (
                -1.0
                * binary.child1.mass
                / mass_transfer_timescale(binary, binary.child1)
            )
            binary.is_mt_stable = True
            ...
            binary.mass_transfer_rate = (
                -1.0
                * binary.child2.mass
                / mass_transfer_timescale(binary, binary.child2)
            )
            binary.is_mt_stable = True
        ...
        if Js >= Jb / 3.0:
            ...
            binary.mass_transfer_rate = -1.0 * star.mass / dynamic_timescale(star)
            binary.is_mt_stable = False
            ...
            if REPORT_MASS_TRANSFER_STABILITY:
                print("Mass transfer stability: Donor1 stable ")
            binary.mass_transfer_rate = (
                -1.0 * star.mass / mass_transfer_timescale(binary, star)
            )
            binary.is_mt_stable = True
...
def mass_transfer_timescale(binary, star):
    ...
    if not star.is_star:
        sys.exit(
            "error in mass transfer timescale:  type of system unknown, donor star is not a star"
        )
...
def Rx_wright11(mass, p_rot):
    ...
    tau_conv = 10 ** (
        1.16
        - 1.49 * np.log(mass.value_in(units.MSun))
        - 0.54 * (np.log(mass.value_in(units.MSun))) ** 2
    )
    Ro = p_rot / tau_conv
    if Ro > Ro_sat:
        B = -2.70
...
def blackbody(wavel, T):
    ...
    B_l = (2 * h * c**2 / wavel**5) / (
        np.exp(h * c / (wavel * KB * T), dtype=np.float128) - 1
    )
    return B_l
...
def xuv_luminosity(star):
    ...
    if star.stellar_type.value in [
        10,
        11,
        12,
    ]:  # we have a WD, we integrate a Black Body from 1 nm to 91.2 nm
        F_xuv = integrate.quad(
            blackbody, 1e-09, 9.12e-08, args=(star.temperature.value_in(units.K))
        )[0]
        ...
        if star.mass <= 2 | units.MSun:
            ...
            L_X = L_bol * Rx_wright11(
                star.mass, p_rot_star
            )  # Rossby number approach, Wright 2011
            ...
            elif star.stellar_type.value in [
                3,
                4,
                5,
                6,
                9,
            ]:  # during giant phases, rossby approach again, having convective envelopes
                p_rot_star = (
                    2 * np.pi / star.spin_angular_frequency.value_in(1 / units.s)
                )
...
def flux_inst(t, r_plan, a_st_i, P_plan, P_binary, lum, star_number, i_orb):
    ...
    st_ang = (
        2 * np.pi * t / P_binary + star_number * np.pi
    )  # star phase angle (on plane)
    d_z2 = (r_plan * np.sin(phi) * np.sin(i_orb)) ** 2
    d_p2 = (r_plan * np.cos(phi) - a_st_i * np.cos(st_ang)) ** 2 + (
        r_plan * np.sin(phi) * np.cos(i_orb) - a_st_i * np.sin(st_ang)
    ) ** 2
    distance_sq = d_p2 + d_z2
    return lum / distance_sq


def mass_lost_due_to_evaporation_tertiary(
    stellar_system, dt, outer_planet, inner_binary, self
):
    if REPORT_FUNCTION_NAMES:
    ...
    xi = (
        roche_radius_dimensionless(outer_planet.mass, M_bin)
        * a_pl
        / outer_planet.radius
    )
    ...
    F1 = integrate.quad(
        ...
        t_end,
        args=(
            r_pl.value_in(units.RSun),
            a_st_1.value_in(units.RSun),
            P_pl.value_in(units.Myr),
            P_bin.value_in(units.Myr),
            L_xuv1_erg_s,
            0,
            i_orbits,
        ),
    ...
    Flux_XUV = (F1 + F2) / (4 * np.pi * (t_end - t_start)) | (
        units.erg / units.s / units.RSun**2
    )
    eta = 0.2  # evaporation efficiency parameter
    M_dot = (
        eta
        * np.pi
        * (outer_planet.radius) ** 3
        * Flux_XUV
        / (constants.G * outer_planet.mass * K_Erk)
    )
    mass_lost = M_dot * dt
    return mass_lost
