...
def plot_function(triple, dir_plots):
    ...
    plot_e.plot(times_array_Myr, e_in_array, label=r"$e_\mathrm{in}$")
    plot_e.plot(times_array_Myr, e_out_array, label=r"$e_\mathrm{out}$")
    plot_e.set_xlim(0, t_max_Myr)
    plot_e.set_xlabel(r"$t/\mathrm{Myr}$")
    plot_e.set_ylabel("$e$")
    plot_e.legend(loc=0)

    plot_i_relative.plot(times_array_Myr, i_relative_array * 180.0 / np.pi)
    plot_i_relative.set_xlim(0, t_max_Myr)
    plot_i_relative.set_ylim(
        0.9 * min(i_relative_array * 180.0 / np.pi),
        1.1 * max(i_relative_array * 180.0 / np.pi),
    )
    plot_i_relative.set_xlabel(r"$t/\mathrm{Myr}$")
    plot_i_relative.set_ylabel(r"$i_\mathrm{relative} ({}^\circ)$")

    plot_a_in.plot(times_array_Myr, a_in_array_AU)
    plot_a_in.set_xlabel(r"$t/\mathrm{Myr}$")
    plot_a_in.set_ylabel(r"$a_\mathrm{in}$")

    plot_a_out.plot(times_array_Myr, a_out_array_AU)
    plot_a_out.set_xlabel(r"$t/\mathrm{Myr}$")
    plot_a_out.set_ylabel(r"$a_\mathrm{out}$")
    ...
    plot_e_in.plot(times_array_Myr, e_in_array, label=r"$e_\mathrm{in}$")
    plot_e_in.set_xlim(0, t_max_Myr)
    plot_e_in.set_xlabel(r"$t/\mathrm{Myr}$")
    plot_e_in.set_ylabel("$e$")
    plot_e_in.legend(loc=0)

    plot_e_out.plot(times_array_Myr, e_out_array, label=r"$e_\mathrm{out}$")
    plot_e_out.set_xlim(0, t_max_Myr)
    plot_e_out.set_xlabel(r"$t/\mathrm{Myr}$")
    ...
    plot_e_in.set_xlim(0, t_max_Myr)
    plot_e_in.set_xlabel(r"$t/\mathrm{Myr}$")
    plot_e_in.set_ylabel(r"$e_\mathrm{in}$")
    ...
    plot_e_in_g_in.set_xlabel(r"$\cos(g_\mathrm{in})$")
    plot_e_in_g_in.set_ylabel(r"$e_\mathrm{in}$")
    ...
    dyn_inst = (
        2.8
        / (1 - e_out_array)
        * (1 - 0.3 * i_relative_array / np.pi)
        * (
            (1 + m3_array / (m1_array + m2_array))
            * (1 + e_out_array)
            / (np.sqrt(1 - e_out_array))
        )
        ** 0.4
        / (a_out_array_AU / a_in_array_AU)
    )
    octupole = (
        (m1_array - m2_array)
        / (m1_array + m2_array)
    ...
    semiseq = (
        ...
        * m3_array
        / (m1_array + m2_array)
        * (a_in_array_AU / a_out_array_AU / (1 - e_out_array)) ** 3
    ) / np.sqrt(1 - e_in_array)

    plt.semilogy(times_array_Myr, dyn_inst, ".")
    plt.semilogy(times_array_Myr, octupole, ".")
    ...
    a_in_final_theory = (
        a_in_array_AU[0] * (m1_array[0] + m2_array[0]) / (m1_array + m2_array)
    )  # wind
    ...
    corot_spin_inner = (
        1.0 / np.sqrt(a_in_array_AU**3 / (m1_array + m2_array)) * constants
    )
    corot_spin_outer = (
        1.0 / np.sqrt(a_out_array_AU**3 / (m1_array + m2_array + m3_array)) * constants
    )
    # FIXME SR: needs to be done with units!
    # RSun_in_AU = 0.00464913034382
    rsun_in_au = (1 | units.RSun).value_in(units.au)
    ...
    constants_Jorb = 2.9071938904e11  # [RSun**2 * MSun * Myr**-1]
    J_orb2 = (
        m1_array**2
        * m2_array**2
        / (m1_array + m2_array)
    ...
    plt.plot(times_array_Myr, delta_e_in_array, ".")
    plt.xlabel(r"$t/\mathrm{Myr}$")
    plt.ylabel(r"$\Delta e_{\mathrm{in}}$")
    ...
    plot_e_out.set_xlim(0, t_max_Myr)
    plot_e_out.set_xlabel(r"$t/\mathrm{Myr}$")
    plot_e_out.set_ylabel(r"$e_\mathrm{out}$")

    plot_i_relative2.plot(times_array_Myr, i_relative_array * 180.0 / np.pi)
    plot_i_relative2.set_xlim(0, t_max_Myr)
    plot_i_relative2.set_ylim(
        0.9 * min(i_relative_array * 180.0 / np.pi),
        1.1 * max(i_relative_array * 180.0 / np.pi),
    )
    plot_i_relative2.set_xlabel(r"$t/\mathrm{Myr}$")
    plot_i_relative2.set_ylabel(r"$i_\mathrm{relative} ({}^\circ)$")

    plot_e_out_g_out.plot(np.cos(g_out_array), e_out_array)
    plot_e_out_g_out.set_xlabel(r"$\cos(g_\mathrm{out})$")
    plot_e_out_g_out.set_ylabel(r"$e_\mathrm{out}$")
